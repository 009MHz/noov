...
class TestNoovoAPI:
    ...
    async def test_noovo_list_success(self, noovo_api: HomeClient):
        ...
        if noovo_data:
            with step("Validate first item data structure"):
    ...
    async def test_nonexistent_item_error(self, noovo_api: HomeClient):
        ...
        with step("Send GET request for non-existent item"):
    ...
    async def test_coordinates_validation(self, noovo_api: HomeClient):
        with step("Get noovo data for coordinate validation"):
            response, noovo_data, validation_errors = (
                await noovo_api.get_validated_data()
            )

        with step("Validate response and data structure"):
            self._assert_response_ok(response)
            assert (
                not validation_errors
            ), f"Structure validation errors: {validation_errors}"
        ...
        with step("Validate coordinates for all items"):
    ...
    async def test_response_time_performance(self, noovo_api: HomeClient):
        ...
        with step("Measure API response time"):
            response, _, response_time = await noovo_api.get_parsed_response(
                measure_time=True
            )

        with step("Validate response and performance"):
            self._assert_response_ok(response)
            assert (
    ...
    async def test_multiple_requests_workflow(self, noovo_api: HomeClient):
        with step("Execute workflow with multiple API requests"):
            list_response, first_item, detail_response = (
                await noovo_api.execute_workflow()
            )

        with step("Validate list response"):
            self._assert_response_ok(list_response, "List request")

        if first_item is not None:
            with step("Validate first item structure"):
                assert isinstance(first_item, dict), "First item should be a dictionary"

                if detail_response.status == 200:
                    with step("Validate detail response"):
                    ...
                    with step("Validate 404 response"):
            ...
            with step("Validate empty response"):
                assert (
    ...
    async def test_all_noovo_structure(self, noovo_api: HomeClient):
        ...
        with step("Validate all items structure"):
            for i, item in enumerate(noovo_data):
