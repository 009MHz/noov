...
class TestFooter:
    ...
    async def test_footer_link_navigation(
        self, home, platform, context, link_type, path
    ):
        with step(f"Click {link_type} link and wait for new tab"):
        ...
        with step("Switch to the new opened tab"):
            new_page = await new_page_info.value
            await new_page.wait_for_load_state("networkidle")

        with step("Verify the URL redirection"):
            assert new_page.url.endswith(
                path
            ), f"Expected URL to end with {path}, but got {new_page.url}"
